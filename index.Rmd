...
<img src="plots/pollen_sage.gif" width="300"/> <img src="plots/pollen_sage_R1.gif" width="300"/> <img src="plots/pollen_sage_gam20.gif" width="300"/>
...
col_crab <- col
col_crab[sketches$word!="crab"] <- "grey"
d_crab <- sk_5[order((sketches$word == "crab")),]
col_crab <- col_crab[order((sketches$word == "crab"))]

col_crab <- col
col_crab[!(sketches$word %in% c("crab", "kangaroo"))] <- "grey"
d_crab <- sk_5[order((sketches$word %in% c("crab", "kangaroo"))),]
col_crab <- col_crab[order((sketches$word %in% c("crab", "kangaroo")))]

col_cactus <- col
col_cactus[sketches$word!="cactus"] <- "grey"
d_cactus <- sk_5[order((sketches$word == "cactus")),]
col_cactus <- col_cactus[order((sketches$word == "cactus"))]

col_ff <- col
col_ff[sketches$word!="flip flops"] <- "grey"
d_ff <- sk_5[order((sketches$word == "flip flops")),]
col_ff <- col_ff[order((sketches$word == "flip flops"))]

col_banana <- col
col_banana[!(sketches$word %in% c("banana", "boomerang"))] <- "grey"
d_banana<- sk_5[order(sketches$word %in% c("banana", "boomerang")),]
col_banana <- col_banana[order(sketches$word %in% c("banana", "boomerang"))]


render(sk_5, planned_tour(bases), display_slice(axes="off", col=col, v_rel=1, half_range = 5), "png", "gif/slices-%03d.png", apf=0.1, frames = d[3], rescale = FALSE)
render(sk_5, planned_tour(bases), display_xy(axes="off", col=col, half_range = 5), "png", "gif/projection-%03d.png", apf=0.1, frames = d[3], rescale = FALSE)
render(d_crab, planned_tour(bases), display_slice(axes="off", col=col_crab, v_rel=1, half_range = 5), "png", "gif/crabs_slice-%03d.png", apf=0.1, frames = d[3], rescale = FALSE)
render(d_cactus, planned_tour(bases), display_slice(axes="off", col=col_cactus, v_rel=1, half_range = 5), "png", "gif/cactus_slice-%03d.png", apf=0.1, frames = d[3], rescale = FALSE)
render(d_banana, planned_tour(bases), display_slice(axes="off", col=col_banana, v_rel=1, half_range = 5), "png", "gif/banana_slice-%03d.png", apf=0.1, frames = d[3], rescale = FALSE)
render(d_ff, planned_tour(bases), display_slice(axes="off", col=col_ff, v_rel=1, half_range = 5), "png", "gif/ff_slice-%03d.png", apf=0.1, frames = d[3], rescale = FALSE)

render(d_crab, planned_tour(bases), display_xy(axes="off", col=col_crab, half_range = 5), "png", "gif/crabs_proj-%03d.png", apf=0.1, frames = d[3], rescale = FALSE)
render(d_cactus, planned_tour(bases), display_xy(axes="off", col=col_cactus, half_range = 5), "png", "gif/cactus_proj-%03d.png", apf=0.1, frames = d[3], rescale = FALSE)
render(d_banana, planned_tour(bases), display_xy(axes="off", col=col_banana, half_range = 5), "png", "gif/banana_proj-%03d.png", apf=0.1, frames = d[3], rescale = FALSE)
render(d_ff, planned_tour(bases), display_xy(axes="off", col=col_ff, half_range = 5), "png", "gif/ff_proj-%03d.png", apf=0.1, frames = d[3], rescale = FALSE)
...
Side-by-side gif of grand tour - slice tour - sage tour
First slide showing everything, second slide highlighting one or two groups

---
# Summary

- New displays make tour more useful for large data
- We can visualize convex shapes (e.g. surfaces of geometric shapes), small features near the center of a distribution (as seen with the Pollen data) and better resolve grouping structures in large datasets (for example the different hand-drawn sketches)
- Both displays are accessible through dedicated `display_` and `animate_` functions in the `tourr` package
- Both displays rely on tuning parameters and would benefit from an interactive interface
